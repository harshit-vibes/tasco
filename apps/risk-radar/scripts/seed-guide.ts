/**
 * Seed Risk Radar Guide Content to DynamoDB
 *
 * This script seeds the app guide content (feature showcase / help carousel)
 * to the centralized DynamoDB storage.
 *
 * Run with: bun run scripts/seed-guide.ts
 */

import { putAppGuide, type CreateAppGuideInput } from "@tasco/db";

const APP_ID = "risk-radar";

// English guide content
const englishGuide: CreateAppGuideInput = {
  appId: APP_ID,
  language: "en",
  appName: "Risk Radar",
...
const vietnameseGuide: CreateAppGuideInput = {
  appId: APP_ID,
  language: "vi",
...
async function seedGuides() {
  console.log("🚀 Seeding Risk Radar guides to DynamoDB...\n");

  try {
    // Seed English guide
    console.log("📝 Seeding English guide...");
    const enGuide = await putAppGuide(englishGuide);
    console.log(`   ✅ English guide created: ${enGuide.id}`);

    // Seed Vietnamese guide
    console.log("📝 Seeding Vietnamese guide...");
    const viGuide = await putAppGuide(vietnameseGuide);
    console.log(`   ✅ Vietnamese guide created: ${viGuide.id}`);

    console.log("\n✨ All guides seeded successfully!");
    console.log(`\n📊 Summary:`);
    console.log(`   - App: ${APP_ID}`);
    console.log(`   - Languages: en, vi`);
    console.log(`   - Slides per language: ${englishGuide.slides.length}`);
  } catch (error) {
